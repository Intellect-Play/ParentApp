...
import {setPlayerNames} from '../../../src/redux/games/shadowGuess/shadowGuessSlice';

const PlayerShadowNames = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const numberOfPlayers = useSelector(
    state => state.shadowGuess.numberOfPlayers,
  );
  const playerNames = useSelector(state => state.shadowGuess.playerNames);
        ...
        <View style={styles.buttonContainer}>
          <ColorGuessButton
            ...
            onPress={() => {
              ...
              setError('');
              navigation.navigate('DifficultyShadowGuessPage');
            }}
          />
          {error ? (
...
export default PlayerShadowNames;
