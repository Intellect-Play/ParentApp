...
import {setPlayerNames} from '../../../src/redux/games/flagGuess/flagGuessSlice';

const PlayerFlagNames = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const numberOfPlayers = useSelector(state => state.flagGuess.numberOfPlayers);
  const playerNames = useSelector(state => state.flagGuess.playerNames);
        ...
        <View style={styles.buttonContainer}>
          <ColorGuessButton
            ...
            onPress={() => {
              ...
              setError('');
              navigation.navigate('DifficultyFlagGuessPage');
            }}
          />
          {error ? (
...
export default PlayerFlagNames;
